...
public:
    ...
    bool try_push(void* frame) noexcept {
        ...
        for (;;) {
            ...
            const auto difference = static_cast<std::int64_t>(sequence - pos);
    ...
    void* pop() noexcept {
        ...
        const std::uint64_t sequence =
            slot.sequence.load(std::memory_order_acquire);
        const auto difference = static_cast<std::int64_t>(sequence - (pos + 1));
...
public:
    ...
    bool push(void* item, std::uint8_t* spawn_mode = nullptr) noexcept {
    ...
    void* pop() noexcept {
        for (;;) {
            ...
            void* item =
                buf_[h & kLocalQueueMask].load(std::memory_order_relaxed);
    ...
    std::uint32_t grab(void** out, std::uint32_t max) noexcept {
        for (;;) {
            ...
            for (std::uint32_t i = 0; i < n; ++i) {
                out[i] = buf_[(h + i) & kLocalQueueMask].load(
                    std::memory_order_relaxed);
            }
            std::uint32_t expected = h;
            if (head_.compare_exchange_strong(expected, h + n,
                                              std::memory_order_seq_cst,
                                              std::memory_order_relaxed)) {
                return n;
            }
    ...
    std::uint32_t pop_half(void** out) noexcept {
        ...
        for (std::uint32_t i = 0; i < n; ++i) {
            out[i] =
                buf_[(h + i) & kLocalQueueMask].load(std::memory_order_relaxed);
        }
        if (!head_.compare_exchange_strong(h, h + n, std::memory_order_seq_cst,
                                           std::memory_order_relaxed)) {
            return 0;  // a thief beat us to it; it already took work off our
                       // hands
        }
        return n;
    }
...
public:
    ...
    std::uint32_t pop_batch(void** out, std::uint32_t max,
                            std::uint32_t share) {
        if (empty()) return 0;
        std::lock_guard<std::mutex> lock(mutex_);
